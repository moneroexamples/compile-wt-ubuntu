...
You can compile either latest version of the source code from [Wt github's page](https://github.com/kdeforche/wt), or tagged version. For the bleeding edge features, use the latest version. However, this might
be not stable and buggy. For stable version, compile tagged version of the
source code, e.g., v3.3.5.

### Latest version of the source code
Having the dependencies, we can download the current github Wt version and compile it as follows:
...
Please not that this is a current version of Wt on github, **not the official and stable
release**. Thus, as the development of Wt continues virtually on daily basis, sometimes
...
Wt which can be found [here](https://github.com/kdeforche/wt/releases/tag/3.3.5).

### Stable tagged version
The latest version of the source code can contain many bugs, as its not yet
officially released. Thus, one can compile official tagged version, e.g., **3.3.5**:
...
├── serialization
│   ├── binary_archive.h
│   ├── binary_uFull `/opt/bitmonero-dev/` tree is [here](https://github.com/moneroexamples/compile-monero-09-on-xubuntu-16-04-beta-1/blob/master/res/full_tree_bitmonero-dev.txt).
tils.h
│   ├── crypto.h
│   ├── debug_archive.h
│   ├── json_archive.h
